...
std::tuple<Eigen::Matrix4f, unsigned> MotionEstimatorPnP::Estimate(Frame& frame1, Frame& frame2)
{
    ...
    if (matches> MIN_INLIERS)
    {
        ...
        if (inliersPnP.size() >= MIN_INLIERS)
		{
            ...
            const auto [errMean1, errVar1] = CalcReprojectionError(frame2.GetImage(), pts3d1, pts2d2, Rmat1, tvec, 2.0f, reprojectedInliers);
            ...
            if (needRefine)
            {
                ...
                const auto [errMean, errVar] = CalcReprojectionError(frame2.GetImage(), points3d1Inliers, points2d2Inliers, RmatNew, refinedTvec, 2.0f, refinedInliers);
